...
unsigned int DynamixelClass::readIRFiredata(unsigned char ID, unsigned char sensorNum){
    Instruction_Packet_Array[0] = ID;
    Instruction_Packet_Array[1] = READ_FIREDATA_LENGTH;
    Instruction_Packet_Array[2] = COMMAND_READ_DATA;
    Instruction_Packet_Array[3] = RAM_IR_FIREDATA1_L + ((sensorNum - 1) * 2);
    Instruction_Packet_Array[4] = READ_TWO_BYTE_LENGTH;
    ...
    if (Status_Packet_Array[2] == 0){                                           // If there is no status packet error return value
        return (Status_Packet_Array[4] << 8) | Status_Packet_Array[3];  // Return value
    }else{
        return (Status_Packet_Array[2] | 0xF000);                           // If there is a error Returns error value
    }
}

unsigned int DynamixelClass::setRAMIRTreshold( unsigned char ID, unsigned char sensorNum, int Treshold){

    Instruction_Packet_Array[0] = ID;
    Instruction_Packet_Array[1] = SET_TRESHOLD_LENGTH;
    Instruction_Packet_Array[2] = COMMAND_WRITE_DATA;
    Instruction_Packet_Array[3] = RAM_IR_TRESHOLD1_L + ((sensorNum - 1) * 2);
    Instruction_Packet_Array[4] = byte(Treshold);
    Instruction_Packet_Array[5] = byte(Treshold >> 8);
...
unsigned int DynamixelClass::setEEPROMIRTreshold( unsigned char ID, unsigned char sensorNum, int Treshold){
    Instruction_Packet_Array[0] = ID;
    Instruction_Packet_Array[1] = SET_TRESHOLD_LENGTH;
    Instruction_Packet_Array[2] = COMMAND_WRITE_DATA;
	if (sensorNum < 6)
		Instruction_Packet_Array[3] = EEPROM_IR_TRESHOLD1_L+ ((sensorNum - 1) * 2);
	else
		Instruction_Packet_Array[3] = EEPROM_IR_TRESHOLD6_L+ ((sensorNum - 1) * 2);
    Instruction_Packet_Array[4] = byte(Treshold);
    Instruction_Packet_Array[5] = byte(Treshold >> 8);
...
unsigned int DynamixelClass::readIRDetected(unsigned char ID){

    Instruction_Packet_Array[0] = ID;
    Instruction_Packet_Array[1] = READ_DETECTED_LENGTH;
    Instruction_Packet_Array[2] = COMMAND_READ_DATA;
    Instruction_Packet_Array[3] = RAM_IR_OBJECT_DETECTED;
    Instruction_Packet_Array[4] = READ_ONE_BYTE_LENGTH;
    ...
    if (Status_Packet_Array[2] == 0){                   // If there is no status packet error return value
        return Status_Packet_Array[3];                  // Return IR detected value (7 in 1)
    }else{
        return (Status_Packet_Array[2] | 0xF000);   // If there is a error Returns error value
    }
}
