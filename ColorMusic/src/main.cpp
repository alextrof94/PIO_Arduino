...
// ----- настройки радуги
float RAINBOW_STEP = 1.00;         // шаг изменения цвета радуги
...
#if REMOTE_TYPE == 3
#define BUTT_UP     0xC9801E32
#define BUTT_DOWN   0xA64EB032
#define BUTT_LEFT   0xC5782232
#define BUTT_RIGHT  0x7641B32
#define BUTT_OK     0x4F6E4B32
#define BUTT_1      0xBAFF0132
#define BUTT_2      0x97CD9332
#define BUTT_3      0xF8E2FE32
#define BUTT_4      0x21C3BC32
#define BUTT_5      0xFE924E32
#define BUTT_6      0x5FA7B932
#define BUTT_7      0x40ED2E32
#define BUTT_8      0x1DBBC032
#define BUTT_9      0x7ED12B32
#define BUTT_0      0x883A7332
#define BUTT_STAR   0xE94FDE32  // *
#define BUTT_HASH   0xAB6BE132   // #
#endif
...
#define LOG_OUT 1
#include "./../lib/FHT/FHT.h"         // преобразование Хартли
...
int lmssii;
void HIGHS() {
  for (lmssii = 0; lmssii < NUM_LEDS; lmssii++) 
    leds[lmssii] = CHSV(HIGH_COLOR, 255, thisBright[2]);
}
void MIDS() {
  for (lmssii = 0; lmssii < NUM_LEDS; lmssii++) 
    leds[lmssii] = CHSV(MID_COLOR, 255, thisBright[1]);
}
void LOWS() {
  for (lmssii = 0; lmssii < NUM_LEDS; lmssii++) 
    leds[lmssii] = CHSV(LOW_COLOR, 255, thisBright[0]);
}
void SILENCE() {
  for (lmssii = 0; lmssii < NUM_LEDS; lmssii++) 
    leds[lmssii] = CHSV(EMPTY_COLOR, 255, EMPTY_BRIGHT);
}
...
int aaii;
void analyzeAudio() {
  for (aaii = 0 ; aaii < FHT_N ; aaii++) {
    int sample = analogRead(SOUND_R_FREQ);
    fht_input[aaii] = sample; // put real data into bins
  }
...
byte animib;
int animii;
int endsOf5 [4] = {NUM_LEDS / 5, NUM_LEDS / 5 * 2, NUM_LEDS / 5 * 3, NUM_LEDS / 5 * 4};
int endsOf3 [2] = {NUM_LEDS / 3, NUM_LEDS / 3 * 2};
int endsOf2 = NUM_LEDS / 2 - 1;
void animation() {
  // согласно режиму
  switch (this_mode) {
    case 0: // UV
      {
        count = 0;
        for (animii = (MAX_CH - 1); animii > ((MAX_CH - 1) - Rlenght); animii--) {
          leds[animii] = ColorFromPalette(myPal, (count * index));   // заливка по палитре " от зелёного к красному"
          count++;
        }
        count = 0;
        for (animii = (MAX_CH); animii < (MAX_CH + Llenght); animii++ ) {
          leds[animii] = ColorFromPalette(myPal, (count * index));   // заливка по палитре " от зелёного к красному"
          count++;
        }
        if (EMPTY_BRIGHT > 0) {
          animThisDark = CHSV(EMPTY_COLOR, 255, EMPTY_BRIGHT);
          for (animii = ((MAX_CH - 1) - Rlenght); animii > 0; animii--)
            leds[animii] = animThisDark;
          for (animii = MAX_CH + Llenght; animii < NUM_LEDS; animii++)
            leds[animii] = animThisDark;
        }
      }
      break;
    case 1: // UV rainbow
      {
        ...
        count = 0;
        for (animii = (MAX_CH - 1); animii > ((MAX_CH - 1) - Rlenght); animii--) {
          leds[animii] = ColorFromPalette(RainbowColors_p, (count * index) / 2 - hue);  // заливка по палитре радуга
          count++;
        }
        count = 0;
        for (animii = (MAX_CH); animii < (MAX_CH + Llenght); animii++ ) {
      ...
      break;
    case 2: // low-mid-high 5
      {
        for (animii = 0; animii < endsOf5[0]; animii++) {
            leds[animii] = CHSV(HIGH_COLOR, 255, thisBright[2]);
        }
        for (animii = endsOf5[0]; animii < endsOf5[1]; animii++) {
            leds[animii] = CHSV(MID_COLOR, 255, thisBright[1]);
        }
        for (animii = endsOf5[1]; animii < endsOf5[2]; animii++) {
            leds[animii] = CHSV(LOW_COLOR, 255, thisBright[0]);
        }
        for (animii = endsOf5[2]; animii < endsOf5[3]; animii++) {
            leds[animii] = CHSV(MID_COLOR, 255, thisBright[1]);
        }
        for (animii = endsOf5[3]; animii < NUM_LEDS; animii++) {
            leds[animii] = CHSV(HIGH_COLOR, 255, thisBright[2]);
        }
      }
      break;
    case 3: // low-mid-high 3
      {
        for (animii = 0; animii < endsOf3[0]; animii++) {
          leds[animii] = CHSV(HIGH_COLOR, 255, thisBright[2]);
        }
        for (animii = endsOf3[0]; animii < endsOf3[1]; animii++) {
          leds[animii] = CHSV(MID_COLOR, 255, thisBright[1]);
        }
        for (animii = endsOf3[1]; animii < NUM_LEDS; animii++) {
          leds[animii] = CHSV(LOW_COLOR, 255, thisBright[0]);
        }
      }
      break;
    case 4: // low-mid-high 1
      {
    ...
    case 5: // strobe
      {
        if (strobe_bright > 0) 
        {
          for (animii = 0; animii < NUM_LEDS; animii++) 
            leds[animii] = CHSV(STROBE_COLOR, STROBE_SAT, strobe_bright);
        }
        else 
        {
          for (animii = 0; animii < NUM_LEDS; animii++) 
            leds[animii] = CHSV(EMPTY_COLOR, 255, EMPTY_BRIGHT);
        }
      }
      break;
    case 6: // single color
      {
        switch (light_mode) {
          case 0: 
            for (animii = 0; animii < NUM_LEDS; animii++) 
              leds[animii] = CHSV(LIGHT_COLOR, LIGHT_SAT, 255);
            break;
          case 1:
            ...
            for (animii = 0; animii < NUM_LEDS; animii++) 
              leds[animii] = CHSV(this_color, LIGHT_SAT, 255);
            break;
          case 2:
            if (millis() - rainbow_timer > 30) {
              rainbow_timer = millis();
            ...
            for (animii = 0; animii < NUM_LEDS; animii++) {
              leds[animii] = CHSV((int)floor(rainbow_steps), 255, 255);
    ...
    case 8: // spectre
      {
        animHueIndex = HUE_START;
        for (animii = 0; animii < NUM_LEDS / 2; animii++) {
          animThisBright = map(freq_f[(int)floor((NUM_LEDS / 2 - animii) / freq_to_stripe)], 0, freq_max_f, 0, 255);
          animThisBright = constrain(animThisBright, 0, 255);
          leds[animii] = CHSV(animHueIndex, 255, animThisBright);
          leds[NUM_LEDS - animii - 1] = leds[animii];
          animHueIndex += HUE_STEP;
          if (animHueIndex > 255) 
            animHueIndex = 0;
        }
      }
  ...
  if (ONstate) {
    if (millis() - main_timer > MAIN_LOOP) {
      ...
      if (this_mode == 0 || this_mode == 1) {
        for (ib = 0; ib < 100; ib++) {                                 // делаем 100 измерений
        ...
        if (EMPTY_BRIGHT > 5) {
          for (ii = 0; ii < NUM_LEDS; ii++)
            leds[ii] = CHSV(EMPTY_COLOR, 255, EMPTY_BRIGHT);
        }
      ...
      if (this_mode == 2 || this_mode == 3 || this_mode == 4 || this_mode == 7 || this_mode == 8) {
        ...
        colorMusic[2] = 0;
        for (ii = 0 ; ii < 32 ; ii++) {
          if (fht_log_out[ii] < SPEKTR_LOW_PASS) fht_log_out[ii] = 0;
        }
        // низкие частоты, выборка со 2 по 5 тон (0 и 1 зашумленные!)
        for (ib = 2; ib < 6; ib++) {
          if (fht_log_out[ib] > colorMusic[0]) colorMusic[0] = fht_log_out[ib];
        }
        // средние частоты, выборка с 6 по 10 тон
        for (ib = 6; ib < 11; ib++) {
          if (fht_log_out[ib] > colorMusic[1]) colorMusic[1] = fht_log_out[ib];
        }
        // высокие частоты, выборка с 11 по 31 тон
        for (ib = 11; ib < 32; ib++) {
          if (fht_log_out[ib] > colorMusic[2]) colorMusic[2] = fht_log_out[ib];
        }
        freq_max = 0;
        for (ib = 0; ib < 30; ib++) {
          if (fht_log_out[ib + 2] > freq_max) freq_max = fht_log_out[ib + 2];
          if (freq_max < 5) freq_max = 5;

          if (freq_f[ib] < fht_log_out[ib + 2]) freq_f[ib] = fht_log_out[ib + 2];
          if (freq_f[ib] > 0) freq_f[ib] -= LIGHT_SMOOTH;
          else freq_f[ib] = 0;
        }
        freq_max_f = freq_max * averK + freq_max_f * (1 - averK);
        for (ib = 0; ib < 3; ib++) {
          colorMusic_aver[ib] = colorMusic[ib] * averK + colorMusic_aver[ib] * (1 - averK);  // общая фильтрация
          colorMusic_f[ib] = colorMusic[ib] * SMOOTH_FREQ + colorMusic_f[ib] * (1 - SMOOTH_FREQ);      // локальная
          if (colorMusic_f[ib] > ((float)colorMusic_aver[ib] * MAX_COEF_FREQ)) {
            thisBright[ib] = 255;
            colorMusicFlash[ib] = true;
            running_flag[ib] = true;
          } else colorMusicFlash[ib] = false;
          if (thisBright[ib] >= 0) thisBright[ib] -= SMOOTH_STEP;
          if (thisBright[ib] < EMPTY_BRIGHT) {
            thisBright[ib] = EMPTY_BRIGHT;
            running_flag[ib] = false;
          }
        }
        animation();
      }
