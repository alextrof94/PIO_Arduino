...
const uint8_t PIN_BTN_INFO = 4;
const uint8_t PIN_ENABLE_DEVICES = 6;
const uint8_t PIN_POWER_LEVEL = A0;
const uint8_t PIN_RFID_RST = 9;
const uint8_t PIN_RFID_SDA = 10;
const uint8_t PIN_RFID_MOSI = 11;
const uint8_t PIN_RFID_MISO = 12;
const uint8_t PIN_RFID_SCK = 13;

const uint8_t batteryCount = 5;
const uint8_t PIN_PH_BATTERYS[batteryCount] = {A1, A2, A3, A6, A7};


bool workByBattery = true;
uint32_t timerBeforeSleep = 60000;
const uint32_t timerBeforeSleepDelay = 60000;

void wakingUp() {
  digitalWrite(PIN_ENABLE_DEVICES, 1);
  timerBeforeSleep = millis() + timerBeforeSleepDelay;
}

void goToSleep() {
  digitalWrite(PIN_ENABLE_DEVICES, 0);
  attachInterrupt(0, wakingUp, LOW);
...
void setup() {
  ...
  pinMode(PIN_BTN_INFO, INPUT_PULLUP);
  pinMode(PIN_ENABLE_DEVICES, OUTPUT);
  pinMode(PIN_POWER_LEVEL, INPUT);
  for (uint8_t i = 0; i < batteryCount; i++)
    pinMode(PIN_PH_BATTERYS[i], INPUT);
}
